...
const SECTIONS = [
  { folder: "blogs", label: "Blogs", posts: readBlogPosts },
  { folder: "memos", label: "Memos", posts: readMemoPosts },
] as const;
...
export async function renderOgAssets(root = process.cwd()): Promise<OgAssets> {
  ...
  images.set("og/default.png", await renderCard(fonts, avatarSrc, "Crystaworld"));

  for (const section of SECTIONS) {
    for (const post of section.posts(root)) {
      const file = `og/${section.folder}/${post.id}.png`;
      const pathname = `/${section.folder}/${post.id}`;
      images.set(file, await renderPost(fonts, avatarSrc, post, section.label));
      articles[pathname] = {
        title: post.title,
...
async function renderPost(fonts: OgFont[], avatarSrc: string, post: OgBlogPost, label: string): Promise<Buffer> {
  return renderCard(fonts, avatarSrc, post.title, label, formatOgDate(post.date), post.tags);
}
...
async function renderCard(
  ...
  const svg = await satori(
    <OgCard title={title} avatarSrc={avatarSrc} label={label} meta={meta} tags={tags} />,
    {
      width: 1200,
      height: 630,
      fonts,
